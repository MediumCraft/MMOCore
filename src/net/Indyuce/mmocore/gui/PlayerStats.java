...
public class PlayerStats extends EditableInventory {
		...
		if (function.startsWith("profession_")) {
			...
			return new InventoryPlaceholderItem(config) {
				...
				public Placeholders getPlaceholders(PluginInventory inv, int n) {
					...
					double ratio = (double) inv.getPlayerData().getCollectionSkills().getExperience(profession) / MMOCore.plugin.configManager.getNeededExperience(inv.getPlayerData().getCollectionSkills().getLevel(profession) + 1);
		...
		if (function.equals("stats"))
			return new InventoryPlaceholderItem(config) {
				...
				public Placeholders getPlaceholders(PluginInventory inv, int n) {
					...
					for (StatType stat : StatType.values()) {
						double extra = stats.getExtraStat(stat);
						double base = stats.getBase(stat);
	...
	public class PlayerProfileItem extends InventoryPlaceholderItem {
		...
		public Placeholders getPlaceholders(PluginInventory inv, int n) {
			...
			int nextLevelExp = MMOCore.plugin.configManager.getNeededExperience(data.getLevel() + 1);
