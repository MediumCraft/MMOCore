...
public class RPGPlaceholders extends PlaceholderExpansion {
	...
	public String onRequest(@Nullable OfflinePlayer player, @NotNull String identifier) {
		...
		if (identifier.equals("level"))
			...
			return String.valueOf(playerData.getCollectionSkills()
			...
			for (double j = 1; j < 20; j++)
				format.append(ratio >= j ? MMOCore.plugin.configManager.staminaFull : ratio >= j - .5
					? MMOCore.plugin.configManager.staminaHalf : MMOCore.plugin.configManager.staminaEmpty).append(AltChar.listSquare);
			return format.toString();
		}
